...
class ChannelAttention(nn.Module):
    """Learnable channel attention for EEG spatial features"""
...
class TemporalConvBlock(nn.Module):
    """Lightweight multi-scale temporal feature extraction"""
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # Simple parallel convs with different kernels
        self.conv3 = nn.Conv2d(in_channels, out_channels//2, (1, 3), padding=(0, 1))
        self.conv7 = nn.Conv2d(in_channels, out_channels//2, (1, 7), padding=(0, 3))
    ...
    def forward(self, x):
        out3 = self.conv3(x)
        out7 = self.conv7(x)
        out = torch.cat([out3, out7], dim=1)
        out = self.bn(out)
        return self.act(out)


class NovelEEGNet(nn.Module):
    """
    Novel EEGNet variant with:
    1. Channel Attention (spatial attention)
    ...
    def __init__(self, n_classes=4, n_channels=22):
        ...
        # Block 1: Temporal convolution + Depthwise
        ...
        # Block 2: Multi-scale temporal + Separable conv
        self.temporal_conv = TemporalConvBlock(32, 48)
        self.separable = nn.Conv2d(48, 48, (1, 15), padding=(0, 7), bias=False)
        self.bn3 = nn.BatchNorm2d(48)
        self.pool2 = nn.AvgPool2d((1, 8))
        self.drop2 = nn.Dropout(0.35)
        self.channel_attn2 = ChannelAttention(48)

        # Block 3: Additional conv for deeper features
        self.conv4 = nn.Conv2d(48, 64, (1, 5), padding=(0, 2), bias=False)
        ...
        # Classifier with residual-like skip connection in FC
        self.fc = nn.Sequential(
            nn.Linear(64 * 8, 256),
            nn.LayerNorm(256),
            nn.GELU(),
